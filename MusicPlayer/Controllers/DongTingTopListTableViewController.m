//
//  DongTingTopListTableViewController.m
//  MusicPlayer
//
...
@interface DongTingTopListTableViewController () <TryListenManagerDelegate, DownloadManagerDelegate>
...
@implementation DongTingTopListTableViewController
...
{
    // 初始化搜索参数
    self.params = [@{@"id":self.idStr, @"page":@"1", @"size":@"20"} mutableCopy];
    
    [DongTingUtils requestDongTingTopListWithParams:self.params Completion:^(id obj) {
        self.musics = obj;
...
- (void)loadMoreData
{
    int page = [self.params[@"page"] intValue];
    page++;
    self.params[@"page"] = @(page).stringValue;
    [DongTingUtils requestDongTingTopListWithParams:self.params Completion:^(id obj) {
        if (!obj || [obj count] == 0) {
